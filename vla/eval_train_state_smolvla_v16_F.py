#!/usr/bin/env python3
# SmolVLA 학습 상태 종합 평가 스크립트 v2-F (체크포인트 Top5 종합순위 추천)
# 작성일: 2026-08-04
# 기반: eval_train_state_smolvla_v15_E.py (v2-E)
# 작성자: 빅맨 / ZETA Satellite Robotics
#
# ═══════════════════════════════════════════════════════════════════════
# v2-E → v2-F 주요 변경점 (모델별 최고 체크포인트 1~5위 추천)
# ═══════════════════════════════════════════════════════════════════════
#
# [핵심 추가: 평가 6을 "종합 점수 기반 Top5 순위"로 재설계]
#   기존 v2-E: 평가 6에서 state 분산 단일 지표로 "최고 1개"만 추천
#   신규 v2-F: 각 체크포인트를 5개 지표로 채점 → 종합 점수 Top5 추천
#
# [왜 단일 지표가 아니라 종합 점수인가 — 글로벌 사이트 분석 2026]
#   ⚠️ arXiv 2108.03298 "What Matters in Learning from Offline Demos":
#      "validation loss 최고점 또는 마지막 체크포인트로 정책을 고르면
#       최선의 정책보다 10~100% 성능이 낮을 수 있다 → 오프라인 지표 필요"
#   ⚠️ arXiv 2606.00253 "Per-Group Error, Not Total MSE" (SmolVLA 실증):
#      "total MSE 하나만으로는 실기 배포 체크포인트 선택 기준이 못 된다.
#       잔차 오차를 지배하는 관절 그룹이 모델/학습마다 달라진다"
#   ✅ arXiv 2306.00942 TOTO "Train Offline, Test Online":
#      "검증셋 expert 시연 대비 joint-angle / EE-pose MSE가 낮아질수록
#       실제 reward가 오른다 → action MSE는 유효한 오프라인 지표"
#   ✅ arXiv 2108.03298: 최고 loss도 마지막 ckpt도 아닌 '중간 sweet spot'가 최선
#
# [v2-F 종합 순위 채점 방식 — 5개 지표 가중 합산]
#   각 체크포인트마다 아래를 측정하고 0~100 정규화 점수로 환산:
#     (1) 검증셋 action MSE       (가중 x0.35) ← TOTO 근거, 낮을수록↑ (핵심)
#     (2) 공간 적응력(state 분산)   (가중 x0.25) ← Mode collapse 방지, 높을수록↑
#     (3) 5문장 언어 일관성        (가중 x0.15) ← 언어 접지, 낮을수록↑
#     (4) Vision 반응(image 분산)   (가중 x0.15) ← Mode averaging 방지, 적정값↑
#     (5) Chunk 안전성(범위초과=0)  (가중 x0.10) ← 안전, 초과 없을수록↑
#   → 총점 상위 5개를 순위표로 출력 + 추론용 경로 복붙 제공
#   → 검증셋(DATASET_DIR)이 없으면 MSE 항목은 자동 제외하고 나머지로 채점
#
# ═══════════════════════════════════════════════════════════════════════
# [이전 v2-E 이력 — v15 new_merged_100 검증용]
# ═══════════════════════════════════════════════════════════════════════
#
...
LOG_FILE = os.path.join(LOG_DIR, f"smolvla_eval_v2f_top5_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log")
...
print("=" * 70)
print("  SmolVLA 학습 상태 종합 평가  v2-F  (체크포인트 Top5 종합순위)")
print(f"  체크포인트: {CKPT_DIR.split('/')[-1]}")
print(f"  실전 관측: ✅ 색상 구분 성공 / ❌ 10번 중 2번만 'arm(팔)' grasp")
print(f"  주 목표: 여러 지표 종합 → 최고 체크포인트 1~5위 추천")
print(f"  채점 지표: 검증MSE·공간적응력·언어일관성·Vision반응·Chunk안전")
...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# ★ v2-F 신규: 체크포인트 Top5 종합순위용 유틸
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━

def build_validation_samples(max_samples: int = 60):
    """검증셋(DATASET_DIR)에서 (관측→정답 action) 샘플을 추출.
    TOTO(arXiv 2306.00942) 근거: 검증셋 expert action 대비 MSE는 유효한 오프라인 지표.
    실제 이미지가 파이프라인 상 로드가 어려우면 state/action만으로도 근사 가능하도록
    observation.state → 다음 action 매핑을 사용한다(간이 BC 오프라인 오차).
    반환: [(state_tensor[1,6], gt_action[6]), ...]  없으면 빈 리스트.
    """
    samples = []
    try:
        import pandas as pd
        ...
        # 에피소드를 골고루 훑어 다양한 자세 샘플 확보
        eps = sorted(df["episode_index"].unique())
        picked = 0
        for ep_idx in eps:
            ep = df[df["episode_index"] == ep_idx]
            states = np.stack(ep["observation.state"].values)   # [T,6]
            actions = np.stack(ep["action"].values)             # [T,6]
            T = len(states)
            if T < 3:
                continue
            # 에피소드당 3개 지점(초/중/후반) 샘플
            for frac in (0.2, 0.5, 0.8):
                t = int(T * frac)
                t = min(max(t, 0), T - 1)
                samples.append((states[t], actions[t]))
                picked += 1
                if picked >= max_samples:
                    return samples
        return samples
    except Exception as e:
        print(f"  ⚠️  검증셋 샘플 추출 실패: {str(e)[:60]}")
        return samples


def eval_ckpt_val_mse(policy_obj, preprocess_obj, val_samples, img_for_ctx):
    """검증셋 샘플에 대해 예측 action vs 정답 action의 평균 MSE 반환.
    낮을수록 좋음. val_samples 없으면 None.
    """
    if not val_samples:
        return None
    errs = []
    for st_vec, gt_act in val_samples:
        st = torch.tensor([st_vec]).float().cuda()
        try:
            pred = get_action(policy_obj, preprocess_obj, img_for_ctx, img_for_ctx,
                              st, task=TASK_DEFAULT)
            errs.append(float(np.mean((pred - gt_act) ** 2)))
        except Exception:
            continue
    if not errs:
        return None
...
def _norm_lower_better(x, lo, hi):
    """낮을수록 좋은 지표를 0~100 점수로 (lo 이하=100, hi 이상=0)."""
    if x is None:
        return None
    if hi <= lo:
        return 50.0
...
def _norm_higher_better(x, lo, hi):
    """높을수록 좋은 지표를 0~100 점수로 (hi 이상=100, lo 이하=0)."""
    if x is None:
        return None
    if hi <= lo:
        return 50.0
...
def _norm_band(x, lo_good, hi_good, hard_lo, hard_hi):
    """적정 구간(band)에 들면 100, 벗어날수록 감점.
    Vision 반응처럼 '너무 낮아도(둔감) 너무 높아도(과민) 나쁜' 지표용.
    """
    if x is None:
        return None
    if lo_good <= x <= hi_good:
        return 100.0
    if x < lo_good:
        return _norm_higher_better(x, hard_lo, lo_good)
    return _norm_lower_better(x, hi_good, hard_hi)
...
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
# ★★★ 평가 6 (v2-F 전면 재설계): 체크포인트 종합 점수 Top5 순위 추천
# ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
print("\n" + "★" * 70)
print("▶ 평가 6: ⭐⭐ 체크포인트 종합 점수 Top5 순위 추천 (v2-F 핵심)")
print("  각 체크포인트를 5개 지표로 채점 → 가중 합산 → 최고 1~5위 추천")
print("  ─────────────────────────────────────────────────────────")
print("  (1) 검증셋 action MSE   x0.35  낮을수록↑  [TOTO 2306.00942]")
print("  (2) 공간 적응력(state)  x0.25  높을수록↑  [Mode collapse 방지]")
print("  (3) 5문장 언어 일관성   x0.15  낮을수록↑  [언어 접지]")
print("  (4) Vision 반응(image)  x0.15  적정구간↑  [Mode averaging 방지]")
print("  (5) Chunk 안전(범위초과) x0.10  없을수록↑  [실기 안전]")
print("  근거: 단일 지표(loss/MSE)만 쓰면 최선 대비 10~100%↓ [2108.03298]")
print("★" * 70)

# 채점 가중치
W = {"mse": 0.35, "state": 0.25, "cons": 0.15, "vision": 0.15, "chunk": 0.10}

# 검증셋 샘플 준비 (있으면 MSE 채점, 없으면 자동 제외 후 재정규화)
val_samples = build_validation_samples(max_samples=60)
if val_samples:
    print(f"\n  ✅ 검증셋 샘플 {len(val_samples)}개 확보 → action MSE 채점 활성화")
else:
    print(f"\n  ⚠️  검증셋 샘플 없음 → MSE 항목 제외, 나머지 4지표로 채점")

# 평가할 체크포인트: 너무 많으면 최근 12개로 제한(속도)
ckpts_to_eval = ckpts[-12:] if len(ckpts) > 12 else ckpts
print(f"  평가 대상 체크포인트: {len(ckpts_to_eval)}개 → {ckpts_to_eval}\n")

# 순서 의존성 제거: collection_state가 없더라도 자체 정의
try:
    collection_state
except NameError:
    collection_state = torch.tensor([[2.5, -88., 82., 63., -2., 0.7]]).float().cuda()

# 공간 적응력 측정용 state 세트 (평가 9와 동일 취지)
rank_states = [
    torch.tensor([[2.5, -88., 82., 63., -2., 0.7]]).float().cuda(),
    torch.tensor([[-20., -85., 85., 65., -5., 0.7]]).float().cuda(),
    torch.tensor([[20., -85., 85., 65., 5., 0.7]]).float().cuda(),
    torch.tensor([[0., -70., 70., 50., 0., 0.7]]).float().cuda(),
    torch.tensor([[0., -100., 100., 75., 0., 0.7]]).float().cuda(),
]
rank_imgs = test_imgs[:4]  # Vision 반응/MSE 컨텍스트용

SAFE_RANK = [(-180, 180), (-200, 200), (-180, 180), (-180, 180), (-180, 180), (-30, 30)]

# ── 1차: 각 체크포인트에서 원시 지표 측정 ──
raw = []  # dict per ckpt
for ckpt_name in ckpts_to_eval:
    ...
    try:
        ...
        # (1) 검증셋 action MSE
        mse = eval_ckpt_val_mse(p, pp, val_samples, rank_imgs[0])

        # (2) 공간 적응력: state 변이 시 chunk 최종점 분산 (높을수록 좋음)
        st_finals = []
        for st in rank_states:
            reset_policy_state(p)
            b = pp({CAM_WRIST: rank_imgs[0], CAM_TOP: rank_imgs[0],
                    "observation.state": st, "task": [TASK_DEFAULT]})
            with torch.no_grad():
                c = p.predict_action_chunk(b)[0].cpu().numpy()
            st_finals.append(c[-1])
        state_std = float(np.std(np.array(st_finals), axis=0).mean())

        # (4) Vision 반응: image 변이 시 chunk 최종점 분산 (적정 구간)
        img_finals = []
        for img in rank_imgs:
            reset_policy_state(p)
            b = pp({CAM_WRIST: img, CAM_TOP: img,
                    "observation.state": collection_state, "task": [TASK_DEFAULT]})
            with torch.no_grad():
                c = p.predict_action_chunk(b)[0].cpu().numpy()
            img_finals.append(c[-1])
        vision_std = float(np.std(np.array(img_finals), axis=0).mean())

        # (3) 5문장 언어 일관성: 같은 입력에 5문장 action pairwise 평균차 (낮을수록 좋음)
        acts5 = []
        for tk in TASKS_YELLOW:
            a = get_action(p, pp, rank_imgs[0], rank_imgs[0], collection_state, task=tk)
            acts5.append(a)
        acts5 = np.array(acts5)
        pd5 = [np.abs(acts5[i] - acts5[j]).max()
               for i in range(5) for j in range(i + 1, 5)]
        cons = float(np.mean(pd5))

        # (5) Chunk 안전: 범위초과 건수 (0이 최선)
        reset_policy_state(p)
        b = pp({CAM_WRIST: rank_imgs[0], CAM_TOP: rank_imgs[0],
                "observation.state": collection_state, "task": [TASK_DEFAULT]})
        with torch.no_grad():
            csafe = p.predict_action_chunk(b)[0].cpu().numpy()
        oor = sum(1 for t in range(len(csafe)) for j in range(6)
                  if not (SAFE_RANK[j][0] <= csafe[t, j] <= SAFE_RANK[j][1]))

        raw.append({
            "ckpt": ckpt_name, "mse": mse, "state": state_std,
            "vision": vision_std, "cons": cons, "oor": oor,
        })
        mse_str = f"{mse:.5f}" if mse is not None else "  —  "
        print(f"  step {ckpt_name:>8} | MSE {mse_str} | state {state_std:6.3f}° | "
              f"vis {vision_std:6.3f}° | cons {cons:6.3f}° | oor {oor}")

        del p, pp
        torch.cuda.empty_cache()
    except Exception as e:
        print(f"  step {ckpt_name:>8} | 로딩/평가 실패: {str(e)[:34]}")

# ── 2차: 원시 지표를 0~100으로 정규화 후 가중 합산 ──
ranked = []
if raw:
    mse_vals = [r["mse"] for r in raw if r["mse"] is not None]
    have_mse = len(mse_vals) > 0
    if have_mse:
        mse_lo, mse_hi = min(mse_vals), max(mse_vals)
        if mse_hi == mse_lo:
            mse_hi = mse_lo + 1e-9

    for r in raw:
        parts = {}
        # (1) MSE — 데이터 기반 min~max 정규화 (낮을수록 100)
        if have_mse and r["mse"] is not None:
            parts["mse"] = _norm_lower_better(r["mse"], mse_lo, mse_hi)
        else:
            parts["mse"] = None
        # (2) 공간 적응력 — collapse 기준: 1.5°(F)~8°(A)를 0~100
        parts["state"] = _norm_higher_better(r["state"], 1.5, 8.0)
        # (3) 언어 일관성 — 0.3°(A)~2.0°(F), 낮을수록 100
        parts["cons"] = _norm_lower_better(r["cons"], 0.3, 2.0)
        # (4) Vision 반응 — 적정 band: 1.0~6.0 최적, 0.3/12.0에서 0점
        parts["vision"] = _norm_band(r["vision"], 1.0, 6.0, 0.3, 12.0)
        # (5) Chunk 안전 — 초과 0이면 100, 많을수록 감점(10건 이상 0)
        parts["chunk"] = _norm_lower_better(float(r["oor"]), 0.0, 10.0)

        # 사용 가능한 항목만 가중 재정규화 (MSE 없으면 나머지 4개로)
        active = {k: v for k, v in parts.items() if v is not None}
        wsum = sum(W[k] for k in active)
        total = sum(parts[k] * W[k] for k in active) / wsum if wsum > 0 else 0.0

        ranked.append({**r, "parts": parts, "score": total})
    ...
    # ── 출력: Top5 순위표 ──
    print("\n  " + "═" * 66)
    print("  🏆 체크포인트 종합 점수 Top5 순위")
    print("  " + "═" * 66)
    print(f"  {'순위':>4} | {'step':>8} | {'종합':>6} | {'MSE':>5} | {'공간':>5} | "
          f"{'언어':>5} | {'Vis':>5} | {'안전':>5}")
    print(f"  {'-' * 66}")
    ...
    for i, r in enumerate(top5):
        ...
        print(f"  {medals.get(i, str(i+1)):>4} | {r['ckpt']:>8} | {r['score']:6.1f} | "
              f"{cell(pt['mse'])} | {cell(pt['state'])} | {cell(pt['cons'])} | "
              f"{cell(pt['vision'])} | {cell(pt['chunk'])}")

    # ── 추론용 경로 복붙 제공 ──
    print("\n  ── 📌 추론에 바로 쓸 Top5 체크포인트 경로 ──")
    for i, r in enumerate(top5):
        print(f"  {medals.get(i, str(i+1))} #{i+1}  (종합 {r['score']:.1f}점)")
        print(f"      {CKPT_DIR}/checkpoints/{r['ckpt']}/pretrained_model")

    best = top5[0]
    print(f"\n  ⭐ 1순위 추천: step {best['ckpt']}  (종합 {best['score']:.1f}점)")
    print(f"     추론 시 CKPT 경로를 이걸로 변경하세요:")
    print(f"     {CKPT_DIR}/checkpoints/{best['ckpt']}/pretrained_model")

    print(f"""
  💡 순위 해석 가이드:
     - 1~2위 차이가 작으면(±3점) 둘 다 실기 테스트 권장 [2108.03298]
     - MSE만 낮고 공간적응력이 낮으면 과적합 의심 (Mode collapse)
     - 검증셋이 없으면 MSE 열이 '—' → 나머지 4지표로 순위 산정됨
     - 최종 판단은 실기 10회 성공률로 확정 (오프라인은 후보 압축용)""")

    scores["best_ckpt"] = best["ckpt"]
    scores["best_ckpt_score"] = float(best["score"])
    scores["top5_ckpts"] = [r["ckpt"] for r in top5]
    scores["top5_scores"] = [round(r["score"], 1) for r in top5]
else:
    print("\n  ⚠️  채점 가능한 체크포인트가 없습니다 (경로/로딩 확인 필요)")
    scores["best_ckpt"] = None
...
print(f"""
  ...
  ── 핵심 숫자 ─────────────────────────────────────
     ...
     추천 1순위 ckpt:          step {scores.get('best_ckpt', '????')}  (종합 {scores.get('best_ckpt_score', 0):.1f}점)
     추천 Top5 (평가 6):       {scores.get('top5_ckpts', [])}
     Top5 종합점수:            {scores.get('top5_scores', [])}
